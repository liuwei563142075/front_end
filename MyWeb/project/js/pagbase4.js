		...
		if(flag == "init"){	//初始时没按上下时
			...
			if(sexNow != sixNum){
				...
				if(sexNow != sixNum){
					...
					if(pageNow >= (pageTotal-(pageSize-1))){
						code += compareCode(pageTotal-(pageSize-1),pageTotal,pageNow+1);
					}else{
						code += compareCode(pageTotal-(pageSize-1),pageTotal,pageTotal-(pageSize-1));
					}
				}
				code += '<li><a href="javascript:void(0)" onclick="getli('+pageNow+','+pageTotal+','+pageSize+',\'down\')">&raquo;</a></li>';
			}else{
